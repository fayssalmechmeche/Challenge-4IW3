function openCategoryCreateModal() {
  fetch(`/category/new`)
    ...
    .then((html) => {
      // Cet ID correspond à l'ID de votre modal dans le template Twig
      const modalBody = document.querySelector("#newCategoryModalContentId");
      modalBody.innerHTML = html;

      toggleModal("newCategoryModalId", "newCategoryModalContentId");
    })
    .catch((error) =>
...
function openCategoryEditModal(categoryId) {
  fetch(`/category/${categoryId}/edit`)
    .then((response) => response.text())
    .then((html) => {
      const modalBody = document.querySelector("#categoryEditModalContent");
      console.log(modalBody);
      modalBody.innerHTML = html;
      toggleModal("categoryEditModal", "categoryEditModalContent");
    })
    .catch((error) =>
      console.error("Erreur lors de la récupération du formulaire:", error)
    );
  ...
  fetch(`/category/${categoryId}`)
    ...
    .then((html) => {
      const modalContent = document.getElementById(
        "categorieDetailsModalContentId"
      );
      modalContent.innerHTML = html;

      toggleModal("categorieDetailsModal", "categorieDetailsModalContentId");
    })
    .catch((error) =>
...
function editFromShow(categoryId) {
  toggleModal("categorieDetailsModal", "categorieDetailsModalContentId");
...
function addClassToElement() {
  //Ce script est propre à ce formulaire : il rajoute une class à la div id category qui ne peut être gérée dynamiquement depuis le form builder. #}
  let formulaDiv = document.getElementById("category");
  if (formulaDiv) {
